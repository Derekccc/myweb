...
const UserActivator = (props) => {
	...
	const confirmBtnOnClick = () => {
		UpdateUserStatus(props.editData);
	};

	const UpdateUserStatus = (_data) => {
		let functionName = "";

		try {
			functionName = props.page + UpdateUserStatus.name;
			...
			http
				.put("api/user/UpdateUserStatus", data, { timeout: 5000 })
				.then((response) => {
					toast.success("User is successfully " + action + "d.");
					props.onReload();
				})
				.catch((err) => {
					toast.error("Failed to " + action + " User. Please try again.1");
